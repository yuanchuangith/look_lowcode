...
function validateMeta(meta: any, expectedDir?: string): string | null {
...
export function loadPageMetadata(outDir: string) {
...
export function assertPartialBaseline(outDir: string, pageCatalog: any[]) {
    ...
    if (errors.length > 0) {
        throw new PartialBaselineError(
            '当前快照不是可安全单页更新的完整基线，请先执行一次全量 cpm pull。',
            errors.slice(0, 20),
        );
    }
...
export function metadataToIndexes(metadata: any[]) {
    ...
    const eventSubs = sorted.flatMap(meta => meta.eventSubscriptions.map((sub: any) => ({
        ...sub,
        pageDir: meta.dir,
    })));
