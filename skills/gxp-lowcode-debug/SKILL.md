...
表、字段、备注、索引、约束或跨表关系问题，先由 Codex 基于任务语义和本地 Schema 证据做关系判断；该快照来自本机可访问的开发库，不代表生产库。

- `search_database_schema` 按表名、字段名和备注定位，`inspect_table_schema` 返回表结构、数据库声明外键和当前可信关系。
- Codex 优先判断：先结合任务语义、业务锚点、字段备注、外键、唯一键和调用上下文提出并解释候选；每个候选必须回到 `inspect_table_schema` 或 `resolve_table_relation` 做证据校验。若语义不足、候选冲突、证据不完整或无法稳定输出结构化候选，则自动回退到 `generate_candidates` 的脚本解析，并继续执行既有数据库验证。
...
- 每次使用推断关系前检查本机否决记录，不访问策略服务器；命中 `rejected` 后立即停止，不重新验证或重建，直到显式恢复。
...
- 本机否决记录无法读取时只展示 `declared_fk` 物理约束，推断关系标记 `尚未确认`。实时验证仍不确定时不得选择一个候选目标。
- 只有用户明确指出某个关系错误，并且当前回答中已有该关系的 `relation_id` 时，才调用 `reject_table_relation`。普通质疑、待确认或查询失败不写永久否决。
- `reject_table_relation` 仅在本机持久保存 relation ID、标准原因码和审计，不上传服务器或跨电脑同步。`restore_table_relation` 恢复后必须重新完成数据验证；恢复版本使本机旧验证失效，同组竞争候选恢复也会使旧唯一性失效。首次使用只导入本机已有的同 scope 旧缓存，不下载服务器历史。
- 通用关系验证只返回计数型证据；具体业务记录仍按既有门禁使用 `get_records` 的索引等值、小列集和小 limit。

输出分别标记：`数据库架构快照`、`数据验证关系`、`实时数据库验证`、`本机用户否决`、`尚未确认`。
